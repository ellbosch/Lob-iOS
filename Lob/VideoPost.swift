...
public class VideoPost: Decodable {
    var id: String
    var title: String
    var league: String
    ...
    var datePosted: Date {
        get {
            ...
            return formatterRawString.date(from: datePostedRaw) ?? Date()
        }
    }
    var dateShort: Date {
        get {
            // grab a short date for table
            ...
            return formatterShort.date(from: "\(year) \(month) \(day)") ?? Date()
        }
    }
    var author: String
    var redditScore: Int
